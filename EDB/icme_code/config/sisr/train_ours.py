...
def main():
    ...
    if rank <= 0:  # normal training (rank -1) OR distributed training (rank 0-7)
        if resume_state is None:
            ...
            os.system("rm ./log_sisr")
            os.symlink(os.path.join(opt["path"]["experiments_root"], ".."), "./log_sisr")
        ...
        if opt["use_tb_logger"] and "debug" not in opt["name"]:
            ...
            tb_logger = SummaryWriter(log_dir="log_sisr/{}/tb_logger/".format(opt["name"]))
    else:
        util.setup_logger(
    ...
    sde = util.EDB(steps=10, device=device)
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            model.feed_data(LQ) # xt, lq, x0
            
            model.training_bridge_losses(current_step, model, x_start=GT)
